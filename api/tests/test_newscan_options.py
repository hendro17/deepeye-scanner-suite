...
def test_build_cmd_maps_no_fake_flags_for_unsupported_fields():
    ...
    assert not any(
        "recon" in part or "subdomain" in part or "secret" in part or part in ("--full-scan", "--quick-scan")
        for part in cmd
    )


def test_ingest_openapi_yaml_v3(client):
    r = client.post("/api/scans/ingest-openapi", json={"filename": "spec.yaml", "content": YAML_SPEC})
    assert r.status_code == 200
    body = r.json()
    assert body["count"] == 2
    assert body["targets"] == [
...
def test_ingest_openapi_json_swagger2(client):
    ...
    r = client.post("/api/scans/ingest-openapi", json={"filename": "spec.json", "content": spec})
    assert r.status_code == 200
    body = r.json()
    assert body["targets"] == ["https://api.example.com/v2/pets", "https://api.example.com/v2/stores"]
    assert body["count"] == 2
...
def test_ingest_openapi_skips_relative_servers_and_dedupes(client):
    spec = json.dumps({
        "openapi": "3.0.0",
        "servers": [{"url": "https://a.example.com"}, {"url": "/relative-only"}, {"url": "https://a.example.com/"}],
        "paths": {"/x": {}, "/y": {}},
    })
    r = client.post("/api/scans/ingest-openapi", json={"filename": "spec.yaml", "content": spec})
    assert r.status_code == 200
    assert r.json()["targets"] == ["https://a.example.com/x", "https://a.example.com/y"]


def test_ingest_openapi_invalid_content(client):
    r = client.post("/api/scans/ingest-openapi", json={"filename": "bad.yaml", "content": "\t: : ["})
    assert r.status_code == 400


def test_ingest_openapi_non_object_spec(client):
    r = client.post("/api/scans/ingest-openapi", json={"filename": "list.json", "content": "[1, 2]"})
    assert r.status_code == 400
