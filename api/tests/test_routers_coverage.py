...
def test_scan_create_validates_depth_threads(client):
    ...
    r = client.post(
        "/api/scans", json={"target_url": "http://example.com", "depth": 11}
    )
    assert r.status_code == 422
    # threads invalid
    r = client.post(
        "/api/scans", json={"target_url": "http://example.com", "threads": 0}
    )
    assert r.status_code == 422
    r = client.post(
        "/api/scans", json={"target_url": "http://example.com", "threads": 51}
    )
    assert r.status_code == 422
...
def test_get_scan_not_found(client):
    ...
    if r.status_code == 200:
        ...
        assert body == {"error": "not found"} or (
            isinstance(body, list) and body[0].get("error") == "not found"
        )
    else:
...
def test_stream_and_findings(client, monkeypatch):
    ...
    with patch(
        "api.services.report_store.get_findings",
        return_value={"total": 0, "vulnerabilities": []},
    ):
...
def test_config_router(client, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "api.routers.config.read_config", lambda: {"scanner": {"depth": 5}}
    )
    ...
    with (
        patch("api.routers.config.read_config", return_value={"a": 1}),
        patch("api.routers.config.mask_config", return_value={"a": 1}),
    ):
...
def test_providers_router(client, monkeypatch):
    with (
        patch(
            "api.services.config_service.get_provider_status",
            return_value=[{"name": "openai", "configured": True}],
        ),
        patch(
            "api.routers.providers.get_provider_status",
            return_value=[{"name": "openai", "configured": True}],
        ),
    ):
    ...
    fake = subprocess.CompletedProcess(
        args=[], returncode=0, stdout='{"ok": false, "error": "unused"}', stderr=""
    )
