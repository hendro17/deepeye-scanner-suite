...
def test_job_to_dict_parses_args(db):
    conn = get_db()
    cur = conn.execute("INSERT INTO jobs (target, args_json, status) VALUES (?, ?, 'pending')", ("http://example.com", json.dumps({"threads": 7}) ))
    conn.commit()
...
def test_finding_to_dict_variants(db):
    conn = get_db()
    cur = conn.execute("INSERT INTO jobs (target, args_json, status) VALUES (?, ?, 'pending')", ("http://x.com", "{}"))
    job_id = cur.lastrowid
    # with cve_refs, ai_summary, false_positive 1
    conn.execute("INSERT INTO findings (job_id, cve_refs, ai_summary, false_positive) VALUES (?, ?, ?, ?)", (job_id, json.dumps(["CVE-1"]), "summary", 1))
    # without cve_refs, false_positive 0
    conn.execute("INSERT INTO findings (job_id, cve_refs, ai_summary, false_positive) VALUES (?, ?, ?, ?)", (job_id, None, None, 0))
    # with cve_refs None and false_positive NULL
    conn.execute("INSERT INTO findings (job_id, cve_refs, ai_summary, false_positive) VALUES (?, ?, ?, ?)", (job_id, None, None, None))
    conn.commit()
    rows = conn.execute("SELECT * FROM findings WHERE job_id=? ORDER BY id", (job_id,)).fetchall()
    conn.close()
